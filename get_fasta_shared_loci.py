...
import sys
file1 = sys.argv[1] #ids list  <isolate ID><tab><stacks/sample ID>  note: isolate ID is normally the name of the fastq file
        ...
        if "#" not in i:
            ...
            if locid in locidict: #only if loci is in selected loci input
                ...
                for j in stackids:
                    ...
                    if sampleid not in idssamplelist:
            ...
            for line in f1:
                if "consensus" in line: #only take consensus sequences lines
                    ...
                    if samplelocus == int(line[2]): #if sample locus id is found in line
                        ...
                        print ">"+str(j)+"_"+str(samplelocus)+" isolate "+str(sampledict[j])+" catalog loci "+str(i)+"\n"+sampleseq+"\n"
                        output.write(">"+str(j)+"_"+str(samplelocus)+" isolate "+str(sampledict[j])+" catalog loci "+str(i)+"\n"+sampleseq+"\n"); #write in the shared loci fasta file, with sampleID_lociID as header
